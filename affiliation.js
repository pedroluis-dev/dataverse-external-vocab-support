...
function affiliationsQuery(str) {
  ...
  fetch("https://api.ror.org/v2/organizations?query=" + str)
  ...
  .then(data => {
	...
	let label = doc.names.length > 0 ? doc.names[0].value : '';
	//let address = ( doc.addresses.length > 0 ? doc.addresses[0].city : '' ) + ', ' + doc.country.country_name;
      let address = ( doc.locations.length > 0 ? doc.locations[0].geonames_details.name : '' ) + ', ' + doc.locations[0].geonames_details.country_name;
    // Add a table row for the doc
      table.innerHTML += 
      '<tr title="' + label + '">' +
        '<td><a href="' + doc.id + '" target="_blank">' + doc.names[0].value + '</a></td>' +
        '<td>' + address + '</td>' +
        '<td>' + 
          '<span ' + 
            'class="btn btn-default btn-xs glyphicon glyphicon-import pull-right" title="import" ' + 
            'onclick="importAffiliationData(\'' + id + '\', \'' + doc.names[0].value  + '\');">' + 
          '</span>' + 
        '</td>' + 
      '</tr>';
    });
...
var authorSelector = '.edit-compound-field:has(input[data-cvoc-parent="authorAffiliation"]';
